...
export default {
  ...
  methods: {
    ...
    wordReset() {
      this.searchQuery = "";
      this.$store.commit("search/WordReset");
    },
    ...
    // 검색어 전체 삭제
    AllDel() {
      this.$store.dispatch("search/AllDelKeyword");
      this.$store.dispatch("search/GetRecent");
    },
    // 검색어 선택 삭제
    DelKeyword(word, index) {
      this.$store.dispatch("search/DelKeyword", { word, index });
    },
    ...
    SearchWord(word) {
      this.allremove();
      console.log(word);
      this.$store.commit("search/setWord", { word });
      if (this.$route.path !== "/search/allsearch") {
        this.$router.push("/search/allsearch");
        console.log("여기들어오잖아");
      } else {
        this.$store.dispatch("search/SearchWord", {
          word,
        });
      }
    },
    ...
    Categorysearch(cate, category) {
      this.$store.dispatch("search/SearchWord", { category: category });
    },
    // 필터 검색
    Datesearch(what, value, kind) {
      ...
      this.$store.dispatch("search/SearchWord", data);
    },
    // 음성인식
    checkApi() {
      if (
    ...
    autoComplete(e) {
      ...
      if (e.keyCode == 13) {
        ...
        if (e.target.value === "") {
          ...
          this.$store.dispatch("search/autoComplete", {
            word: word,
            category: this.data.class,
            timeStamp: timeStamp,
          });
