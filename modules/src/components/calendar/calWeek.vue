...
export default {
  computed: {
    ...mapGetters("calendar",["GetSchedule"]),
  },
  components: {
    Header,
    DateHeader,
  ...
  methods: {
    ...
    Init() {
      ...
      this.$store.dispatch("calendar/CalList", { data: send, which: "week" });
    },
    calendarData(arg) {
      var now = new Date();
    ...
    getWeekNo(v_date_str) {
      var date = new Date();
      console.log(v_date_str,"v_date_str")
      if (v_date_str) {
