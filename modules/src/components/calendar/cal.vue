...
export default {
  ...
  methods: {
    ...
    getMonthOfDays(monthFirstDay, monthLastDate, prevMonthLastDate) {
      ...
      this.$store.dispatch("calendar/CalList", {data,which:"month"});
    ...
    Rest() {
      if (this.premonth !== this.month) {
        ...
        this.$store.dispatch("calendar/Holiday", data);
      }
      return;
    },
    fill(width, number) {
    ...
    async Detail(day, next, last,yuil){
      ...
      result.date = yuil;
      console.log(result)
      await this.$store.commit("calendar/SaveScheduleList",{data,date:result});
