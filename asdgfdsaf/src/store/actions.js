...
export default {
    ...
    GETMailDelay({ state }) {
        GETMailDelay()
            .then((res) => {
                // console.log(res)
                state.mailconfig.delay.config = res.data;
    ...
    GETMailAutoSave({ state }) {
        GETMailAutoSave()
            .then((res) => {
                // console.log(res)
                state.mailconfig.autosave.config = res.data;
    ...
    SignGreetDelete({ state, commit }, { where }) {
        ...
        console.log(state.store.mailconfig.view[`${where}DetailUnid`])

        SignGreetDelete(state.store.mailconfig.view[`${where}DetailUnid`])
            .then((response) => {
                console.log(response, "SignGreetDelete")
    ...
    GreetEdit({ state, commit }, data) {
        console.log(data, "GreetEdit")
        return GreetEdit(data)
            .then(response => {
                console.log(response, "GreetEdit")
                if (response.status !== 200) {
                    return false;
                }
                return true;
    ...
    async GreetDetail({ state, commit }, { unid }) {
        var result = await GreetDetail(unid)
            .then(response => {
                ...
                if (response.status !== 200) {
                    console.log("GreetDetail not working");
                    r.boo = false;
                } else {
                    console.log("detail 잘옴");
                    r.boo = true;
                }
                return r;
            })
        ...
        if (result.boo) {
            // console.log(commit("GreetViewData", response.data))
    ...
    async SignDetail({ state, commit }, { unid }) {
        var result = await SignDetail(unid)
            .then(response => {
                ...
                if (response.status !== 200) {
                    console.log("SignDetail not working");
                    r.boo = false;
                } else {
                    console.log("detail 잘옴");
                    r.boo = true;
