...
export default {
  ...
  methods: {
    Delete(){
      this.$store.dispatch("SignGreetDelete","greet");
    },
    async Modify() {
      this.GetGreetView.body = this.body;
      this.GetGreetView.subject = this.subject;
      this.GetGreetView.default = this.default;
      var result = await this.$store.dispatch("GreetEdit", this.GetGreetView);
      if(result){
